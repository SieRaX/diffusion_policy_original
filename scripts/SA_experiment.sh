...
# and writes dense_eval.npz + figures INSIDE that run dir. RUN is a single-word path
# (safe in zsh). Run the RUN= line together with the command below it.
...
$PY -m diffusion_policy.experiments.spatial_attention_exp1.dense_eval.run_dense_eval \
  run_dir="$RUN" output_dir="$RUN" device=cuda:0
...
# low_dim (per-run) — resolve each run dir and read its dense_eval.npz
RUN=$(ls -td outputs_HDD4/lift_lowdim_abs_ph_reproduction/train_by_seed_flow_matching/seed* | head -1)
$PY -m diffusion_policy.experiments.spatial_attention_exp1.analysis.run_analysis \
  npz="$RUN/dense_eval.npz" output_dir="$RUN"
...
$PY -m diffusion_policy.experiments.spatial_attention_exp1.analysis.run_analysis \
  npz="$IMG/dense_eval.npz" output_dir="$IMG" npz_compare="$LOW/dense_eval.npz"
