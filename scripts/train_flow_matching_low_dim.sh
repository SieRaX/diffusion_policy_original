#!/usr/bin/env zsh
# Usage: bash scripts/train_flow_matching_low_dim.sh abs=<true|false> cuda=<id|cuda:id>
#   abs=true  -> use absolute-action tasks (<task>_lowdim_abs); abs=false -> <task>_lowdim
...
echo $run_dir
echo ${task_config_name}_lowdim${abs}

for seed in $seed_list; do
    echo -e "\033[32m[Training ${task_config_name}_lowdim${abs} | seed=${seed} | ${device}]\033[0m"
    python train.py --config-name=${config_name} \
        task=${task_config_name}_lowdim${abs} \
