...
class FlowMatchingUnetHybridImagePolicy(BaseImagePolicy):
    """
    Flow-matching (rectified-flow) counterpart of DiffusionUnetHybridImagePolicy.

    The robomimic visual obs-encoder + ConditionalUnet1D backbone are identical to
    the diffusion policy; only the generative process is replaced. Training uses a
    rectified-flow interpolation x_t = (1-t)*x0 + t*x1 with velocity target x1 - x0,
    and sampling integrates the ODE dx/dt = v_theta backward from noise to data with
    a fixed number of Euler steps. Continuous time t is scaled by ``time_scale``
    before the sinusoidal step embedding. See FlowMatchingUnetLowdimPolicy for the
    formulation details.
    """
